...
{
   return hotro_Get("SELECT hotro.Id, danhmuchotro.Name as DanhMucId, hotro.LoaiHoTro, hotro.Name, hotro.Skype, hotro.Phone FROM  hotro, danhmuchotro where danhmuchotro.Id=hotro.DanhMucId  ".(($where!='')?(' and '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function hotro_insert($obj)
{
    return exe_query("insert into hotro (DanhMucId,LoaiHoTro,Name,Skype,Phone) values ('$obj->DanhMucId','$obj->LoaiHoTro','$obj->Name','$obj->Skype','$obj->Phone')",'hotro');
}
//
function hotro_update($obj)
{
    return exe_query("update hotro set DanhMucId='$obj->DanhMucId',LoaiHoTro='$obj->LoaiHoTro',Name='$obj->Name',Skype='$obj->Skype',Phone='$obj->Phone' where Id=$obj->Id",'hotro');
}
//
