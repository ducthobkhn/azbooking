...
{
   return config_Get("SELECT config.Id, config.Title, config.Keyword, config.Description, config.Logo, config.Logo_footer, config.Icon, config.Name, config.Address, config.Phone, config.Hotline, config.Hotlien_datve, config.Email, config.Website, config.Skype, config.Yahoo FROM  config ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function config_insert($obj)
{
    return exe_query("insert into config (Title,Keyword,Description,Logo,Logo_footer,Icon,Name,Address,Phone,Hotline,Hotlien_datve,Email,Website,Skype,Yahoo) values ('$obj->Title','$obj->Keyword','$obj->Description','$obj->Logo','$obj->Logo_footer','$obj->Icon','$obj->Name','$obj->Address','$obj->Phone','$obj->Hotline','$obj->Hotlien_datve','$obj->Email','$obj->Website','$obj->Skype','$obj->Yahoo')",'config');
}
//
function config_update($obj)
{
    return exe_query("update config set Title='$obj->Title',Keyword='$obj->Keyword',Description='$obj->Description',Logo='$obj->Logo',Logo_footer='$obj->Logo_footer',Icon='$obj->Icon',Name='$obj->Name',Address='$obj->Address',Phone='$obj->Phone',Hotline='$obj->Hotline',Hotlien_datve='$obj->Hotlien_datve',Email='$obj->Email',Website='$obj->Website',Skype='$obj->Skype',Yahoo='$obj->Yahoo' where Id=$obj->Id",'config');
}
//
