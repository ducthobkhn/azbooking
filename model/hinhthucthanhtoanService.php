...
{
   return hinhthucthanhtoan_Get("SELECT hinhthucthanhtoan.Id, hinhthucthanhtoan.Name, hinhthucthanhtoan.Img, hinhthucthanhtoan.MoTaNgan FROM  hinhthucthanhtoan ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function hinhthucthanhtoan_insert($obj)
{
    return exe_query("insert into hinhthucthanhtoan (Name,Img,MoTaNgan) values ('$obj->Name','$obj->Img','$obj->MoTaNgan')",'hinhthucthanhtoan');
}
//
function hinhthucthanhtoan_update($obj)
{
    return exe_query("update hinhthucthanhtoan set Name='$obj->Name',Img='$obj->Img',MoTaNgan='$obj->MoTaNgan' where Id=$obj->Id",'hinhthucthanhtoan');
}
//
