...
{
   return tieuchi_Get("SELECT tieuchi.Id, tieuchi.Name, tieuchi.Img, tieuchi.MoTaNgan FROM  tieuchi ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function tieuchi_insert($obj)
{
    return exe_query("insert into tieuchi (Name,Img,MoTaNgan) values ('$obj->Name','$obj->Img','$obj->MoTaNgan')",'tieuchi');
}
//
function tieuchi_update($obj)
{
    return exe_query("update tieuchi set Name='$obj->Name',Img='$obj->Img',MoTaNgan='$obj->MoTaNgan' where Id=$obj->Id",'tieuchi');
}
//
