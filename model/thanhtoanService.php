...
{
   return thanhtoan_Get("SELECT thanhtoan.Id, thanhtoan.Name, thanhtoan.Img, thanhtoan.Link FROM  thanhtoan ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function thanhtoan_insert($obj)
{
    return exe_query("insert into thanhtoan (Name,Img,Link) values ('$obj->Name','$obj->Img','$obj->Link')",'thanhtoan');
}
//
function thanhtoan_update($obj)
{
    return exe_query("update thanhtoan set Name='$obj->Name',Img='$obj->Img',Link='$obj->Link' where Id=$obj->Id",'thanhtoan');
}
//
