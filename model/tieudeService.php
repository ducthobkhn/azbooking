...
{
   return tieude_Get("SELECT tieude.Id, tieude.Name FROM  tieude ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function tieude_insert($obj)
{
    return exe_query("insert into tieude (Name) values ('$obj->Name')",'tieude');
}
//
function tieude_update($obj)
{
    return exe_query("update tieude set Name='$obj->Name' where Id=$obj->Id",'tieude');
}
//
