...
{
   return menu_Get("SELECT menu.Id, menu.Name, menu.Title, menu.Keyword, menu.Description FROM  menu ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function menu_insert($obj)
{
    return exe_query("insert into menu (Name,Title,Keyword,Description) values ('$obj->Name','$obj->Title','$obj->Keyword','$obj->Description')",'menu');
}
//
function menu_update($obj)
{
    return exe_query("update menu set Name='$obj->Name',Title='$obj->Title',Keyword='$obj->Keyword',Description='$obj->Description' where Id=$obj->Id",'menu');
}
//
