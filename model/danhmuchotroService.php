...
{
   return danhmuchotro_Get("SELECT danhmuchotro.Id, danhmuchotro.Name FROM  danhmuchotro ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function danhmuchotro_insert($obj)
{
    return exe_query("insert into danhmuchotro (Name) values ('$obj->Name')",'danhmuchotro');
}
//
function danhmuchotro_update($obj)
{
    return exe_query("update danhmuchotro set Name='$obj->Name' where Id=$obj->Id",'danhmuchotro');
}
//
